...
!function ($, w) {
    ...
    */
    ...
    var programType = ""; //WX=微信项目,PC=桌面项目,YZL=云助理项目
    ...
    var jsPath = function () {
        ...
        for (n = 0; n < a.length; n++) {
            var d = a[n].src;
            if (d.indexOf("base.jq.1.0.js") > 0) {
                var e = d.substring(0, d.lastIndexOf("base.jq.1.0.js"));
                if (d.indexOf("?") > 0) {
    ...
    var loadEncryptJs = function () {
        ...
        loadJs(jsPath + "encrypt/mode-ecb-min.js");

    }
    //jquery
    if (isUpIE()) {
        loadJs(jsPath + "layer.m/layer.js");
    }
    else {
        document.write('<link rel="stylesheet" href="' + jsPath + 'artdialog/css/ui-dialog.css"/>');
        document.write('<script type="text/javascript" src="' + jsPath + 'artdialog/dist/dialog-min.js"></script>');
    }
    ...
    var url = function () {
        ...
        for (n = 0; n < a.length; n++) {
            var d = a[n].src;
            if (d.indexOf("/Content/js/") > 0) {
                var e = d.substring(0, d.lastIndexOf("/Content/js") + 1);
                break;
            }
        }
        return e;
    ...
    */
    var showMsg = function (msg, time, yesFun, noFun) {
        ...
        if (isUpIE()) {
            if (isDefine(yesFun) && isFunc(yesFun)) {
                msgJson = {
                    content: msg, shadeClose: false, btn: ['确定', '取消'], yes: function () {
                        layer.closeAll();
                        yesFun();
                    }, no: function () {
                        layer.closeAll();
                ...
                else if (time < 0) msgJson = { content: msg, shadeClose: false };
            ...
            if (isDefine(yesFun) && isFunc(yesFun)) {
                msgJson = {
                    content: msg, okValue: '确定', ok: function () {
                        msgIndex.close().remove();
                        yesFun();
                ...
                if (!isDefine(noFun)) {
                    delete msgJson.cancelValue;
                    delete msgJson.cancel;
                }
            }
            else {
                time = isDefine(time) ? time : 2; //--如果time没有定义，默认2秒后关闭提示信息

                if (time > 0) msgJson = { content: msg };
                else if (time < 0) msgJson = { content: msg };
                else if (time == 0) msgJson = { content: msg, okValue: '确定', ok: function () { msgIndex.close().remove(); } };
            }
            if (isDefine(msgIndex)) msgIndex.close().remove();
            msgIndex = dialog(msgJson);
            msgIndex.showModal();
            if (time > 0) {
                setTimeout(function () {
                    msgIndex.close().remove();
    ...
    var openLoadImg = function () {
        if (isUpIE()) layer.open({ type: 2, shadeClose: false });
        else showMsg("<img src='" + jsPath + "artdialog/loading-1.gif'/>", -1);
    }
    //关闭弹出层
    var closeMsg = function () {
        if (isUpIE()) layer.closeAll();
    ...
    var ajaxPost = function (reqCode, reqData, retSecret) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (rData.IsError) {
                    if (data.RetCode == '-9999') showMsgT(rData.RetMsg, 0); //系统错误信息
    ...
    *@return ajax调用返回的json数据
    */
    var p = function (pm, success, error) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (pm.loadImg) closeMsg();
                isDefine(success) && success != "" ? success(pdata) : sucessF(pdata);
    ...
    */
    var bind = function (data, area, append) {
        ...
        if ($.isArray(data)) {
            for (var n = 0; n < data.length;n++) {
                ohtml += bindProHtml(data[n], area, html);
            }
        }
        else ohtml += bindProHtml(data, area, html);
    ...
    *@return 处理后的html字符串
    */
    var bindProHtml = function (data, area, html) {
            ...
            for (var n = 0; n < sumAreas.length;n++) {
                ...
                if ($.isArray(preData))//数组多个
                {
                    for (var l = 0; l < preData.length;l++) {
                        rHtml += bindProHtml(preData[l], preArea, preHtml);
                    }
                }
                else rHtml += bindProHtml(preData, preArea, preHtml);
    ...
    var check = function (type, value, obj) {
        ...
        switch (type) {
            case "empty":
                ...
                break;
            case "int":
                if (!isInt(value)) showMsgTInput(obj, tip + "必须为整形");
                break;
            case "mobile":
                if (!isMobile(value)) showMsgTInput(obj, tip + "不是合法手机号");
                break;
            case "float":
                if (!isFloat(value)) showMsgTInput(obj, tip + "必须为浮点型");
                break;
            case "card":
    ...
    */
    var upLoadFile = function (formId,fn) {
        ...
        obj.attr("action", url + "UploadFileIntf.ashx");
